...
#define ser (*pser)
 ...
 public:
  SipeedTOF_MSA010_Publisher() : Node("sipeed_tof_ms_a010") {
    ...
    if (!at_ok) {
      std::cerr << "Camera not responding to AT commands. Exiting." << std::endl;
      rclcpp::shutdown();
      return;
    }
    ...
    while (retry_coeff-- > 0) {
      ...
      if (s.find("+COEFF") != std::string::npos) {
        coeff_ok = true;
        break;
      }
      std::this_thread::sleep_for(std::chrono::milliseconds(200));
    ...
    if (!coeff_ok) {
      std::cerr << "AT+COEFF error. Exiting." << std::endl;
      rclcpp::shutdown();
      return;
    }
    ...
    if (ok_pos != std::string::npos && ok_pos + 4 < s.length()) {
        json_str = s.substr(ok_pos + 4);
    }
    
    int retries = 10;
    while (json_str.find("}") == std::string::npos && retries-- > 0) {
        std::string temp;
        ser >> temp;
        json_str += temp;
    }
    std::cout << "parse json: " << json_str << std::endl;
    
    cJSON *cparms = cJSON_ParseWithLength((const char *)json_str.c_str(), json_str.length());
    if (cparms) {
        cJSON *fx = cJSON_GetObjectItem(cparms, "fx");
        cJSON *fy = cJSON_GetObjectItem(cparms, "fy");
        cJSON *u0 = cJSON_GetObjectItem(cparms, "u0");
        cJSON *v0 = cJSON_GetObjectItem(cparms, "v0");
        
        uvf_parms[0] = fx ? ((float)(fx->valueint / 262144.0f)) : 135.0f;
        uvf_parms[1] = fy ? ((float)(fy->valueint / 262144.0f)) : 135.0f;
        uvf_parms[2] = u0 ? ((float)(u0->valueint / 262144.0f)) : 120.0f;
        uvf_parms[3] = v0 ? ((float)(v0->valueint / 262144.0f)) : 85.0f;
        cJSON_Delete(cparms);
    } else {
        std::cout << "Failed to parse JSON, using default intrinsics" << std::endl;
        uvf_parms[0] = 135.0f;
        uvf_parms[1] = 135.0f;
        uvf_parms[2] = 120.0f;
        uvf_parms[3] = 85.0f;
  ...
  void init_transports() {
    ...
    publisher_depth = it.advertise("depth", 1);

    point_cloud_transport::PointCloudTransport pct(shared_from_this());
    publisher_pointcloud = pct.advertise("cloud", 1);
  }
