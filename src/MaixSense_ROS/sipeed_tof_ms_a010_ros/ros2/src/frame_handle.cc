...
__find_header:
  ...
  if (it - 1 != vecChar.begin()) {
    vecChar.erase(vecChar.begin(), it - 1);
  }
  ...
  if (frame_payload_len > 100 * 100) {
    vecChar.erase(vecChar.begin()); // drop invalid sflag_l
    goto __find_header;
  }
  ...
  vecChar.erase(vecChar.begin(), vecChar.begin() + FRAME_HEAD_SIZE + frame_payload_len +
